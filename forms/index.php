<?php 
    ...
    if(isset($_GET['id'])) {
        $sql7 = "SELECT * FROM forms WHERE id = :id LIMIT 1";
        ...
        $form = $check->fetch(PDO::FETCH_ASSOC);    
        $questions = $db->query("SELECT * FROM questions WHERE formId = '{$_GET['id']}'");
    ...
    <div class="container text-white mx-auto" id="creatorCon">
        <div class="form mx-auto text-center">
            <div class="formHead" style="border-bottom: 1px solid white; margin-bottom:20px">
                ...
                <p id="submitFormErrorText" class="text-center" style="color: red"></p>
                <p id="submitFormSuccessText" class="text-center" style="color: green"></p>
                <p style="color: <?php echo $siteInfo['siteThemeColor'] ?>"><?php echo $form['formDescription'] ?></p>
            </div>
            <form id="submitForm">
                    <?php
                        if ($questions->num_rows > 0) {
                            while($q = $questions->fetch_assoc()) { ?>
                                    ...
                                    <?php if($q['type'] == 0) { ?>
                                        <input type="text" name="answers[]" class="bg-transparent inputField" id="q" placeholder="Enter an answer..." required>
                                    <?php } else { ?>
                                        <select name="answers[]" id="q" class="typeSelect" style='margin-top: -12px' required>
                                            <option value="mis1" disabled selected style="color: white;">---Select-an-option---</option>
                                            <?php $lines = explode(",", $q['options']); 
