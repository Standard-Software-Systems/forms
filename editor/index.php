<?php 
    ...
    $form = $connection->prepare("SELECT * FROM forms WHERE id = :id LIMIT 1");
    $form->execute(
        array(
            "id" => $_GET['id'],
        ),
    );
    $formCount = $form->rowCount();
    if($formCount < 1) {
        header("Location: ../");
    }
...
<head>
    ...
    <title>Editor - <?php echo $siteInfo['siteName'] ?></title>
        ...
        <div class="creator-form">
            <form id="editForm">
                ...
                <div class="row g-3" style="border-bottom: 1px solid white;">
                    ...
                    <input type="text" class="bg-transparent" id="formName" name="formName" style="border: 1px solid white; outline: none; border-radius: 3px; color: white; padding: .3vh 1vh; margin-top: -10px;" value="<?php echo $form['formName'] ?>">
                </div>
                <div class="col-md-3 mb-2 mx-auto" id="formDesc1">
                    <label for="formDesc" class="form-label">Form Description</label><br>
                    <textarea type="text" class="bg-transparent" id="formDesc" name="formDesc" rows="2" cols="30" style="border: 1px solid white; outline: none; border-radius: 3px; color: white; padding: .3vh 1vh; margin-top: -15px;"><?php echo $form['formDescription'] ?></textarea>
                </div>
                <div class="col-md-4 mb-3 mx-auto" id="formWeb1">
                    <label for="formWebhook" class="form-label">Form Discord Webhook</label><br>
                    <textarea type="text" class="bg-transparent" id="formWebhook" name="formWebhook" rows="2" cols="35" style="height: 55px;border: 1px solid white; outline: none; border-radius: 3px; color: white; padding: .3vh 1vh; font-size: 12px; margin-top: -15px;"><?php echo $form['webhook'] ?></textarea>
                </div>
            </div>
                <div class="mt-4" style="margin: auto;">
                        ...
                        ?>
                            <div class="mb-3">
                                <label for="questionName" class="form-label text-center">Question <?php echo $curr ?></label><br>
                                <?php if($row['type'] == 0) { ?>
                                <input type="text" class="bg-transparent inputField" id="question" name="question[]"  placeholder="Enter a question..." style="" required value="<?php echo $row['question'] ?>"></input>
                                ...
                                <?php } else { ?>
                                <input type="text" class="bg-transparent inputField" id="question" name="question[]"  placeholder="Enter a question..." style="" required value="<?php echo $row['question'] ?>"></input>
                                <textarea class="questionInput bg-transparent inputField mt-2" placeholder="Seperate each option with comma..." id="options options[]" name="options[]"><?php echo $row['options'] ?></textarea>
                                <select name="type[]" id="typeSelect" class="typeSelect" onchange="getval(this);">
                                    <option value="input">Input Field</option>
                                    <option value="select" selected>Select Menu</option>
                                </select>
                                <?php } ?>
                            </div>
                            <?php $curr++; } ?>
                        </div>
                        <input type="hidden" name="currEditor" id="currEditor" value="<?php echo $questions->rowCount() ?>">
                        <br>  
                        <div class="mb-3">
                            <button id="addQuestions" class="mb-3 " onclick="addQuestionEditor();">Add Question</button>
                ...
                <div class="row g-3 mt-4" style="border-top: 1px solid white;">
                    <button type="submit" name="editForm" class="editForm mt-4">Edit Form</button>
                </div>
             </form>
            </div>
    </div>                
